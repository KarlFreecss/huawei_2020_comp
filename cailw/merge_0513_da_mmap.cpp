...
const int JUMP_PATH_PER_NODE    = 128; //Smaller would be better, like 512
const int second_coef           = 256;

//char out_buff[MAX_ANS_NUM * MAX_PATH_LENGTH * 12];
...
BackwardPath * global_jump_table[THREAD_NUM][JUMP_TABLE_SIZE];
int global_jump_table_size[THREAD_NUM][JUMP_TABLE_SIZE];
            ...
            for (; itr_mid < itr_mid_end && rev_graph[itr_mid] >= head; itr_mid += 2){
                ...
                if (mid != u){
                    ...
                    if (jump_status[mid] == 0){
                        ...
                        const int table_idx = status2idx(jump_status[mid]);
                        const int table_used = status2len(jump_status[mid]);
                        if (jump_table_size[table_idx] == table_used){
                            BackwardPath * tmp_jump_table = jump_table[table_idx];
                            ...
                            if (status2len(jump_status[mid]) > jump_table_size[table_idx]) {
                                cout << "jump table full [2]" << ' ' << jump_table_size[table_idx] << endl;
                                exit(-2);
                            }
                        #endif
                        jump_table[table_idx][table_used] = (BackwardPath){v, u, amount_mid, amount_u};
                        jump_status[mid]++;
                }
            }
...
void run_job(int_std thread_id, int_std graph_size){
    ...
    auto jump_table_size = global_jump_table_size[thread_id];
    for (int i = 0; i < JUMP_TABLE_SIZE; ++i){
        jump_table[i] = new BackwardPath[JUMP_PATH_PER_NODE];
        jump_table_size[i] = JUMP_PATH_PER_NODE;
    }
    ...
    for (int t = 0; t < graph_size; ++t){
        ...
        const int jump_num = init_jump(i, jump_table, jump_table_size, jump_status, init_node);
...
void mmap_write_data(char file_name[], const int graph_size){
    ...
    //_exit(0);
    //int tmp_size = write(fd, out_buff, out_size);
    close(fd);
}
