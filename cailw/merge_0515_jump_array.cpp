...
const int JUMP_PATH_PER_NODE    = 512;
...
NodeInfo global_jump_status[THREAD_NUM][MAX_ID_NUM];
BackwardPath global_jump_table[THREAD_NUM][JUMP_TABLE_SIZE * JUMP_PATH_PER_NODE];
BackwardPath global_tmp_jump_table[THREAD_NUM][JUMP_TABLE_SIZE * JUMP_PATH_PER_NODE];
int global_jump_table_mid_id[THREAD_NUM][MAX_ID_NUM];
...
#define TRY_QUICK_JUMP(head, v, amount_head) if (jump_status[v].first != jump_status[v].last)\
                quick_jump(head, v, jump_table + jump_status[v].first, jump_status[v].last - jump_status[v].first, \
                    node_list, used, thread_id, amount_##v, amount_head)

void search(int_std head,
            BackwardPath *jump_table,
            NodeInfo * jump_status,
            uint_byte * used,
            int_std thread_id,
            NodeInfo * node_info,
            NodeInfo * second_graph_second_info){
    if (jump_status[head].first != jump_status[head].last) head_quick_jump(head, jump_table + jump_status[head].first, jump_status[head].last - jump_status[head].first, thread_id);
    int_std node_list[4];
        ...
        for (; itr_v < itr_end_v; itr_v+= 2){
            ...
            for (;itr_k < itr_end_k; itr_k+=2){
				...
				//  		quick_jump(head, l, jump, node_list, used, thread_id, amount_l, amount_u);
					if (unlikely(jump_status[l].first != jump_status[l].last and l != u and l != v)) {
						quick_jump(head, l, jump_table + jump_status[l].first, jump_status[l].last - jump_status[l].first, \
...
int init_jump(int_std head, 
            BackwardPath * jump_table,
            BackwardPath * tmp_jump_table,
            int * jump_table_mid_id,
            NodeInfo * jump_status,
        ...
        for (; itr_v < itr_v_end && rev_graph[itr_v] > head; itr_v += 2){
            ...
            for (; itr_mid < itr_mid_end && rev_graph[itr_mid] >= head; itr_mid += 2){
                ...
                if (mid != u){
                    ...
                    if (jump_status[mid].last == 0){
                        init_node.push_back(mid);
                    }
                    ++jump_status[mid].last;
                    jump_table_mid_id[jump_num] = mid;
                    tmp_jump_table[jump_num++] = (BackwardPath){v, u, amount_mid, amount_u};
                }
            }
        }
    }
    if (jump_num == 0) return 0;
    sort(init_node.begin(), init_node.end());
    jump_status[init_node[0]].first = 0;
    const int init_node_size = init_node.size();
    for (int i = 1; i < init_node_size; ++i){
        const int idx = init_node[i];
        const int idx_pre = init_node[i-1];
        jump_status[idx].first = jump_status[idx_pre].first + jump_status[idx_pre].last;
        jump_status[idx_pre].last = jump_status[idx_pre].first;
    }
    const int idx = init_node[init_node_size - 1];
    jump_status[idx].last = jump_status[idx].first;
    

    for (int i = 0; i < jump_num; ++i){
        int mid = jump_table_mid_id[i];
        memcpy(jump_table + jump_status[mid].last++, &tmp_jump_table[i], sizeof(BackwardPath));
    }

    for (const auto & mid : init_node){
        const int first = jump_status[mid].first;
        const int last = jump_status[mid].last;
        if (last - first > 1) {sort(jump_table + first, jump_table + last);}
    }
    return jump_num;
}
...
void run_job(int_std thread_id, int_std graph_size){
    ...
    auto tmp_jump_table = global_tmp_jump_table[thread_id];
    auto jump_table_mid_id = global_jump_table_mid_id[thread_id];
    ...
    for(;;){
        ...
        for (int i = aidx_begin; i < aidx_end; ++i) {
            const int jump_num = init_jump(i, jump_table, tmp_jump_table, jump_table_mid_id, jump_status, init_node);
            ...
            if (jump_num > 0){
                ...
                for (const auto & mid : init_node){
                    jump_status[mid].first = 0;
                    jump_status[mid].last = 0;
                }
            }
        }
    }
