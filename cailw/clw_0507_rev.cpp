...
void preprocess(){
    ...
    graph_node_edge_sort<RevEdgeData>(graph, node_info, ids_size);
    for (int i = 0; i < ids_size; ++i){
        node_info[i].last -= 2;
    }
...
#define EDGE_ITR_INIT(u, v)\
                auto itr_##u = node_info[v].last;\
                const auto itr_begin_##u = node_info[v].first;\
                for (; itr_ ##u >= itr_begin_##u; itr_##u-=2) {if (graph[itr_##u] > head) break;} \
                node_info[v].last = itr_ ##u;
