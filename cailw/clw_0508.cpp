...
atomic_long sort_idx(-1);
template<class T>
void graph_node_edge_sort_multi_imp(int_std * graph, NodeInfo * node_info, int valid_graph_size){
    for (int t = 0; t < valid_graph_size; ++t){
        int i = sort_idx += 1;
        if (i >= valid_graph_size) break;
        const int first = node_info[i].first, last = node_info[i].last;
        if (last - first > 2) sort((T *)(graph + first), (T *)(graph + last));
    }
}
...
template<class T>
void graph_node_edge_sort_multi(int_std * graph, NodeInfo * node_info, int valid_graph_size){
    thread threads[THREAD_NUM];
    sort_idx = -1;
    for (int i = 0; i < THREAD_NUM; ++i){
        threads[i] = thread(graph_node_edge_sort_multi_imp<T>, graph, node_info, valid_graph_size);
    }
...
int main(){
    ...
    cout << try_in << ' ' << try_in_s << ' ' << 1 - (try_in_s * 1. / try_in) << endl;
    return 0;
}
