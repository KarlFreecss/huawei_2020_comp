...
void run_job(int_std thread_id, int_std graph_size){
    ...
    for (int i = thread_id; i < graph_size; i+=THREAD_NUM){
        ...
        if (i % THREAD_NUM == thread_id){
            const int jump_num = init_jump(i, jump_pool[thread_id], jump_update_flag, init_node);
            if (jump_num > 0){
                handle_thread_id[i] = thread_id;
                search(i, jump_pool[thread_id], jump_update_flag, used_pool[thread_id], thread_id, thread_first_edge);
            }
        }
