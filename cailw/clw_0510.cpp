...
int ALIGNED in_degree[MAX_ID_NUM];
int ALIGNED out_degree[MAX_ID_NUM];
...
void build_second_graph(){
    ...
    for (int head = 0; head < ids_size; ++head){
        ...
        for (; itr_u < itr_u_end; itr_u += 2){
            ...
            global_second_graph_second_info[node_info_count].first = node_info_second_count;
            ...
            for (; itr_v < itr_v_end; itr_v += 2){
                ...
                if (amount_check(amount_u, amount_v)){
                    second_graph[node_info_second_count] = v;
                    second_graph[node_info_second_count+1] = amount_v;
                    node_info_second_count += 2;
                }
            }
            global_second_graph_second_info[node_info_count++].last = node_info_second_count;
        }
    }
}
...
void preprocess(){
    ...
    #ifdef REQUIRE_DEBUG_INFO
    {
        auto end = std::chrono::steady_clock::now();
        chrono::duration<double> elapsed_seconds = end-start;
        cout << "reid time: " << elapsed_seconds.count() << "s\n";
    }
    #endif
    ...
    #ifdef REQUIRE_DEBUG_INFO
    {
        auto end = std::chrono::steady_clock::now();
        chrono::duration<double> elapsed_seconds = end-start;
        cout << "Preprocess General Time: " << elapsed_seconds.count() << "s\n";
    }
    #endif
...
void write_count(const int graph_size, int_std & total_ans, int_std & out_size){
    ...
    out_size = 0;
    {
        const int len = MIN_PATH_LENGTH;
        const auto idx = len - MIN_PATH_LENGTH;
        const auto & ans = ans_pool[idx];
        int_std start_point_pool[THREAD_NUM] = {0};
        for (int i = 0; i < graph_size; ++i){
            if (ans[i] == 0) {continue;}
            int_std thread_id = handle_thread_id[i];
            auto & start_point = start_point_pool[thread_id];
    ...
    {
        const int len = MAX_PATH_LENGTH - 3;
        const auto idx = len - MIN_PATH_LENGTH;
        const auto & ans = ans_pool[idx];
        int_std start_point_pool[THREAD_NUM] = {0};
        for (int i = 0; i < graph_size; ++i){
    ...
    {
        const int len = MAX_PATH_LENGTH - 2;
        const auto idx = len - MIN_PATH_LENGTH;
        const auto & ans = ans_pool[idx];
        int_std start_point_pool[THREAD_NUM] = {0};
        for (int i = 0; i < graph_size; ++i){
    ...
    {
        const int len = MAX_PATH_LENGTH - 1;
        const auto idx = len - MIN_PATH_LENGTH;
        const auto & ans = ans_pool[idx];
        int_std start_point_pool[THREAD_NUM] = {0};
        for (int i = 0; i < graph_size; ++i){
...
int main(){
    ...
    #ifdef REQUIRE_DEBUG_INFO
    {
        ...
        auto end = std::chrono::steady_clock::now();
        chrono::duration<double> elapsed_seconds = end-start;
        cout << "Input time cost            : " << elapsed_seconds.count() << "s\n";
    }
    #endif
    ...
    #ifdef REQUIRE_DEBUG_INFO
    {
        auto end = std::chrono::steady_clock::now();
        chrono::duration<double> elapsed_seconds = end-start;
        cout << "preprocess time cost       : " << elapsed_seconds.count() << "s\n";
    }
    #endif
