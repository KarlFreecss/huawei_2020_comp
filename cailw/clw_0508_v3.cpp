...
int second_graph_idx[MAX_DATA_RECORD_SIZE * second_coef];
...
char ALIGNED id_str[MAX_ID_NUM][((MAX_ID_STRING_LENGTH) / 8 + 1) * 8];
        ...
        for (; itr_u < itr_u_end; itr_u += 2){
            ...
            for (; itr_v < itr_v_end; itr_v += 2){
                ...
                if (amount_check(amount_u, amount_v)){
                    second_graph_idx[node_info_second_count >> 1] = (itr_v - global_node_info[u].first) >> 1;
...
void search(int_std head,
    ...
    for (;itr_u < itr_end_u; itr_u+=2){
        ...
        for (; itr_v < itr_end_v; itr_v+= 2){
            ...
            //EDGE_ITR_INIT(k, v);
            const auto itr_k_idx = second_graph_info[u].first + second_graph_idx[itr_v >> 1];
            ...
            for (;itr_k < itr_end_k; itr_k+=2){
                ...
                const auto itr_l_idx = second_graph_info[v].first + second_graph_idx[itr_k >> 1];
