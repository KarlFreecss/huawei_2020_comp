...
void loop_check(int graph_size) {
    ...
    while (p != delete_list.end()) {
        ...
        for (int itr_v = v_start; itr_v < v_end; itr_v+=2) {
            ...
            in_degree[u]--;
            if (in_degree[u] == 0 && out_degree[v] > 0) {
...
void graph_rebuild(int * g, NodeInfo * info, int begin_idx, int end_idx) {
...
int build_str(int begin_idx, int end_idx) {
    for (int i = begin_idx; i < end_idx; ++i){
        int id = hash_query(ids[i], id_hash_table_flag, id_hash_table_key, id_hash_table_value);
        id = rebuild_table[id];
        if (id >= 0) {
            const string str = to_string(ids[i]);
            id_len[id] = str.length() + 1;
        
            id_str_r[id][0] = str.length() + 1;
            memcpy(id_str_r[id] + 1, str.c_str(), id_len[i]);
            id_str_r[id][id_len[id]] = '\n';

            id_str_s[id][0] = str.length() + 1;
            memcpy(id_str_s[id] + 1, str.c_str(), id_len[i]);
            id_str_s[id][id_len[id]] = ',';
        }
    }
}
...
int rebuild_thread(int thread_id, int after_delete) {
    ...
    graph_rebuild(graph, global_node_info, begin_inx, end_idx);
    graph_rebuild(rev_graph, rev_node_info, begin_inx, end_idx);
...
void preprocess(){
    ...
    graph_rebuild(graph, global_node_info, after_delete * (THREAD_NUM - 1) / THREAD_NUM, after_delete);
    graph_rebuild(rev_graph, rev_node_info, after_delete * (THREAD_NUM - 1) / THREAD_NUM, after_delete);
