...
#define IMPOSSIBLE(x, v) if ((amount_##x > 5ll * node_info[v].max) or (3ll * amount_##x < node_info[v].min)) continue

#define ALWAYS(x, v) if ((amount_##x <= 5ll * node_info[v].min) and (3ll * amount_##x >= node_info[v].max)) always_check = 1; else always_check = 0
    ...
    for (;itr_u < itr_end_u; itr_u+=2){
        ...
        for (;itr_v < itr_end_v; itr_v+=2){
            ...
            for (;itr_k < itr_end_k; itr_k+=2){
                ...
                EDGE_ITR_INIT(l, k);
                ALWAYS(k, k);
                if (always_check == 1) goto itr_l_without_check;
