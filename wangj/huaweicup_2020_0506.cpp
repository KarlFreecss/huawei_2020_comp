...
const int JUMP_HASH_TABLE_SIZE    = 10007;
...
inline
void jump_hash_clear(int_std id, int thread_id){
    int_std idx = get_jump_hash_idx(id, thread_id);
    jump_hash_table_flag[thread_id][idx] = 0;
}
...
void run_job(int_std thread_id, int_std graph_size){
    ...
    auto jump_thread = jump_table[thread_id];
    auto jump_length_thread = jump_table_length[thread_id];
    auto & used = used_pool[thread_id];
    for (int t = 0; t < graph_size; ++t){
        int i = handle_num += 1;
        if (i >= graph_size){break;}
        const int jump_num = init_jump(i, jump_thread, jump_length_thread, jump_update_flag, init_node, thread_id);
