...
inline
void graph_node_edge_init(NodeInfo * node_info, int * nums, int valid_graph_size){
...
void loop_check(int graph_size) {
    ...
    while (p != delete_list.end()) {
        ...
        for (int itr_v = v_start; itr_v < v_end; ++itr_v) {
            int_std u = graph[itr_v].v;
            --in_degree[u];
...
template<class T>
void graph_rebuild(int thread_id, T * g, NodeInfo * info, int begin_idx, int end_idx) {
    int valid_edge_num = 0;
    for (int i = begin_idx; i < end_idx; i++) {
        int new_last = info[i].first;
        for (int j = info[i].first; j < info[i].last; ++j) {
            if (rebuild_table[g[j].v] >= 0) {
                g[new_last].v = rebuild_table[g[j].v];
                g[new_last].amount = g[j].amount;
                ++new_last;
            }               
        }
